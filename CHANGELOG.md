...
## 2025-10-23 - 画面仕様書自動生成モード追加

### 追加機能
- **Excel画面設計書生成（NEW!）**
  - DOM解析によりボタン・リンク・入力項目を自動収集し、`Screens / ボタン・リンク / 入力項目` の3シート構成で出力
  - 必須判定、バリデーション属性、アクション記述をまとめてドキュメント化
  - スクリーンショットを再撮影し、仕様書と同一フォルダに保存
  - CLI: `node bin/spec-sheet.js` / エイリアス: `msc-spec`
  - MCPツール: `generate_spec_sheet` で利用可能。出力先やファイル名は引数で指定可能
