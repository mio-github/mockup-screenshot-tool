...
function loadConfig(configPath = null) {
  ...
  try {
    ...
    // 相対パスを絶対パスに変換し、足りない設定にデフォルト値を適用
    ...
    if (!config.screenshotsDir) {
      config.screenshotsDir = path.join(config.outputDir, 'screenshots');
    }
    if (!config.annotatedDir) {
      config.annotatedDir = path.join(config.outputDir, 'screenshots-annotated');
    }
    ...
    if (!config.specSheet.outputDir) {
      config.specSheet.outputDir = path.join(config.outputDir, 'specifications');
    }
    if (!config.specSheet.screenshotDir) {
      config.specSheet.screenshotDir = path.join(config.specSheet.outputDir, 'screens');
    }

    if (!config.specSheet.fileName) {
      const baseName = (config.projectName || 'project')
        .toString()
        .trim()
        .toLowerCase()
        .replace(/[^a-z0-9-_]+/g, '-')
        .replace(/-+/g, '-')
        .replace(/^-|-$/g, '') || 'project';
      config.specSheet.fileName = `${baseName}_screen_spec.xlsx`;
    }

    if (config.specSheet.outputDir) {
      config.specSheet.outputDir = resolvePathFromConfig(config.specSheet.outputDir, configDir);
    }
    if (config.specSheet.screenshotDir) {
      config.specSheet.screenshotDir = resolvePathFromConfig(config.specSheet.screenshotDir, configDir);
    }
