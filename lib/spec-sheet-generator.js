...
export async function generateSpecSheet(config, options = {}) {
  ...
  const screensSheet = workbook.addWorksheet('Screens');
  screensSheet.columns = [
    { header: '画面ID', key: 'screenId', width: 20 },
    { header: 'URL / パス', key: 'path', width: 30 },
    { header: '画面タイトル', key: 'title', width: 30 },
    { header: '主な見出し・機能', key: 'features', width: 50 },
    { header: 'カテゴリ', key: 'category', width: 20 },
    { header: '説明', key: 'description', width: 50 },
    { header: 'パンくず / 備考', key: 'breadcrumbs', width: 40 },
    { header: 'スクリーンショット', key: 'screenshot', width: 40 },
  ];

  const actionsSheet = workbook.addWorksheet('ボタン・リンク');
  actionsSheet.columns = [
    { header: '画面ID', key: 'screenId', width: 20 },
    { header: '要素種別', key: 'elementType', width: 15 },
    { header: 'テキスト / ラベル', key: 'label', width: 40 },
    { header: '主要属性', key: 'attributes', width: 35 },
    { header: 'CSSセレクタ', key: 'selector', width: 45 },
    { header: '動作 / 遷移先', key: 'action', width: 40 },
    { header: '備考', key: 'notes', width: 50 },
  ];

  const inputsSheet = workbook.addWorksheet('入力項目');
  inputsSheet.columns = [
    { header: '画面ID', key: 'screenId', width: 20 },
    { header: 'フィールド名', key: 'label', width: 35 },
    { header: '入力タイプ', key: 'inputType', width: 20 },
    { header: 'CSSセレクタ', key: 'selector', width: 45 },
    { header: 'プレースホルダ / 初期値', key: 'placeholder', width: 35 },
    { header: '必須', key: 'required', width: 8 },
    { header: 'バリデーション', key: 'validation', width: 50 },
    { header: '備考', key: 'notes', width: 45 },
  ];

  applyHeaderStyle(screensSheet);
  applyHeaderStyle(actionsSheet);
  ...
  try {
    for (const pageConfig of config.pages) {
      ...
      const relativeScreenshot = path.relative(path.dirname(outputPath), screenshotPath);
      const screenMeta = screenMetaMap.get(pageConfig.name) || {};

      const screenRow = screensSheet.addRow({
        screenId: pageConfig.name,
        path: pageConfig.path,
        title: analysis.title || screenMeta.title || '',
        features: analysis.headings.join('\n'),
        category: screenMeta.category || '',
        description: screenMeta.description || analysis.metaDescription || '',
        breadcrumbs: analysis.breadcrumbs.join(' > '),
        screenshot: relativeScreenshot,
      });
      setWrapAlignment(screenRow, ['features', 'description', 'breadcrumbs', 'screenshot']);
      ...
      for (const button of analysis.buttons) {
        ...
        const actionDescriptions = pickActionDescriptions(button.selector, actionMap);
        const notes = formatNotes(button.datasetNotes.join('\n'), actionDescriptions);

        const actionRow = actionsSheet.addRow({
          screenId: pageConfig.name,
          elementType: 'ボタン',
        ...
        setWrapAlignment(actionRow, ['label', 'attributes', 'selector', 'action', 'notes']);
      }
      ...
      for (const link of analysis.links) {
        ...
        const actionRow = actionsSheet.addRow({
          ...
          action: link.href,
          notes: link.datasetNotes.join('\n'),
        });
        setWrapAlignment(actionRow, ['label', 'attributes', 'selector', 'action', 'notes']);
      }

      for (const field of analysis.inputs) {
        const actionDescriptions = pickActionDescriptions(field.selector, actionMap);
        const validation = buildValidationSummary(field);
        ...
        const notes = formatNotes(notesItems.join('\n'), actionDescriptions);

        const inputRow = inputsSheet.addRow({
          screenId: pageConfig.name,
          label: field.label || '(ラベル未検出)',
          inputType: field.typeAttr || field.tag,
          selector: field.selector,
          placeholder: [field.placeholder, field.value].filter(Boolean).join('\n'),
          required: field.required ? 'はい' : 'いいえ',
          validation,
          notes,
        });
        setWrapAlignment(inputRow, ['label', 'selector', 'placeholder', 'validation', 'notes']);
      }
