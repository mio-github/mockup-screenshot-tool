...
function generateAnnotatedHTML(imagePath, imageFilename, annotationConfig, imageWidth, imageHeight) {
  ...
  const svgAnnotations = items.map((item, index) => {
    const { x, y, text, description, direction } = item;
    ...
    const hasDescription = lines.length > 0 && !(lines.length === 1 && lines[0] === '');
    const descriptionGap = hasDescription ? 10 : 0;
    const descriptionBlockHeight = hasDescription ? descriptionLineHeight * lines.length : 0;
    ...
    return `
      <!-- Annotation ${index + 1}: ${text} -->
      <g class="annotation" data-index="${index}">
        <!-- Line from point to callout -->
        <line x1="${x}" y1="${y}" x2="${calloutX}" y2="${calloutY}"
              stroke="#FB923C" stroke-width="2" stroke-dasharray="5,5" />

        <!-- Point marker -->
        <circle cx="${x}" cy="${y}" r="8" fill="#F97316" stroke="white" stroke-width="2" />

        <!-- Callout box -->
        <rect x="${calloutX - boxWidth/2}" y="${boxTop}"
              width="${boxWidth}" height="${boxHeight}"
              fill="rgba(249, 115, 22, 0.95)" rx="8" stroke="white" stroke-width="2" />

        <!-- Title -->
        <text x="${calloutX}" y="${titleY}"
        ...
        <!-- Description lines -->
        ${lines.map((line, i) => `
          <text x="${calloutX}" y="${descriptionStartY + i * descriptionLineHeight}"
  ...
  if (config.overview && config.overview.enabled) {
    ...
    const sectionsHTML = sections.map(section => {
      ...
      if (section.items && Array.isArray(section.items)) {
        ...
        contentHTML = `<p class="overview-text">${section.content}</p>`;
      }

      return `
        <section class="overview-section">
          <h2 class="overview-heading">${section.heading}</h2>
          ${contentHTML}
        </section>
      `;
    }).join('');
    ...
    overviewHTML = `
      <div class="overview-page">
        <h1 class="overview-title">${config.overview.title || 'システム概要'}</h1>
        ${sectionsHTML}
      </div>
    `;
  }
...
<head>
  ...
  <link href="https://fonts.googleapis.com/css2?family=Noto+Sans+JP:wght@400;500;700&display=swap" rel="stylesheet">
  <style>
    ${getPDFStyles(config.pdfOptions?.layout)}
  </style>
</head>
<body>
  <!-- Cover Page -->
 ...
 */
function getPDFStyles(layoutOptions = {}) {
  const {
    headerMaxHeight = 200,
